...
/**
 * Esta clase representa el controlador de la aplicación diseñada en MVC.
...
public class Controlador implements ActionListener, ItemListener {
	 ...
	 * @param menuP Menú principal del programa, vista principal.
	 */
	...
	/**
	 * Método que gestiona los eventos de botón.
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		// Botón que abre un cuadro de diálogo que pide la confirmación del alta del
		// ticket
		else if (e.getSource().equals(menuCrudT.btnAltaTicket)) {
			dlgAltaTicket.setVisible(true);
		}
		// Botón que elige el ticket una vez seleccionado con el desplegable y muestra
		// la consulta
		...
		// Botón que REALIZA EL ALTA DEL TICKET y muestra la venta para sumar
		// artículos al ticket
		...
		// Botón que selecciona una fila de la tabla
			...
			if (filaSeleccionada >= 0) {
				// Verifica que una fila está seleccionada
		...
		// Botón que da de alta artÃ­culos
			...
			try {
				...
				if (articulos.txtDescripcion.getText().equals("") || articulos.txtPrecio.getText().equals("")
					...
					if (modelo.comprobarExistencia(descripcion)) {
						JOptionPane.showMessageDialog(null, "El artículo ya existe.", "Error",
								JOptionPane.ERROR_MESSAGE);
					} else {
				...
				JOptionPane.showMessageDialog(null, "Formato de entrada no válido.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		}
		...
		// Botón para eliminar artículos
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "El id especificado no es válido.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		}
	...
	/**
	 * Método que gestiona los eventos de listas desplegables.
	 */
	@Override
